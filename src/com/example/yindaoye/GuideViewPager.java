...
public class GuideViewPager extends ViewPager {
	...
	protected void dispatchDraw(Canvas canvas) {
		if (this.bg != null) {
			 ...
			 * getWidth()
			 * 等于ViewPager一个页面的宽度，即手机屏幕宽度。在该计算中可以理解为滑动一个ViewPager页面需要滑动的像素值。
			 * ((width - n) / (count - 1))
			 * /getWidth()也就表示ViewPager滑动一个像素时，背景图片滑动的宽度。 x * ((width - n) /
			 * (count - 1)) / getWidth()也就表示ViewPager滑动x个像素时，背景图片滑动的宽度。
